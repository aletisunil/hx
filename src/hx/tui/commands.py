...
async def cmd_model(ctx: CommandContext, args: str) -> None:
    ...
    chosen = await ctx.app.ask(ModelPicker(models, ctx.session.meta.model, initial=query))
    if chosen:
...
async def _switch_model(ctx: CommandContext, model_id: str) -> None:
    ...
    ctx.app.status.set_model(model_id, subscription=info.is_subscription)
    # Resolved per model, so the depth can change without the user asking: the
    # bar has to say so at the moment it happens.
    ctx.app.status.set_effort(ctx.app.models.displayed_effort(model_id))
    ctx.app.status.set_context(0, info.context_window)
    billing = (
...
async def cmd_effort(ctx: CommandContext, args: str) -> None:
    ...
    from hx.tui.views.pickers import DEFAULT_EFFORT_ROW, EffortPicker
    ...
    if not choice:
        if not info.reasoning_levels:
            ...
            return
        picked = await ctx.app.ask(EffortPicker(info, ctx.app.models.requested_effort))
        if picked is None:
            return
        choice = picked
...
async def cmd_rewind(ctx: CommandContext, args: str) -> None:
    ...
    from hx.tui.views.pickers import RewindPicker
    ...
    chosen = await ctx.app.ask(RewindPicker(points))
    if chosen is None:
        return
...
async def cmd_configure(ctx: CommandContext, args: str) -> None:
    ...
    from hx.tui.views.login import ConfigureDialog
    ...
    key = await ctx.app.ask(
        ConfigureDialog("Configure", _configure_summary(ctx.app, hint), warning)
    )
    if not key:
        return
...
async def cmd_login(ctx: CommandContext, args: str) -> None:
    ...
    from hx.providers import registry
    from hx.tui.views.login import ProviderDialog

    provider_id = args.strip()
    if not provider_id:
        chosen = await ctx.app.ask(ProviderDialog(_provider_rows(registry.SPECS, ctx.app.auth)))
        if not chosen:
            return
        provider_id = chosen
...
def _configure_summary(app: Any, key_hint: str) -> list[tuple[str, str]]:
    """Label/value rows describing the session, for the configure dialog."""
    rows: list[tuple[str, str]] = []
    model = getattr(app, "models", None)
    if model is not None and getattr(model, "current", None):
        rows.append(("model", str(model.current)))
    rows.append(("permissions", str(getattr(app, "mode", "default"))))
    rows.append(("sandbox", str(getattr(app, "sandbox_backend", "") or "none")))
    rows.append(("api key", key_hint))
    return rows


def _provider_rows(specs: Any, auth: Any) -> list[tuple[str, str, bool]]:
    """``(id, label, has_credential)`` for each provider that can be signed in."""
    rows: list[tuple[str, str, bool]] = []
    for spec in specs:
        try:
            present = bool(auth.has_credential(spec.id))
        except Exception:
            present = False
        rows.append((spec.id, spec.label, present))
    return rows
