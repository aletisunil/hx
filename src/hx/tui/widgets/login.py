...
class LoginModal(ModalScreen[None]):
    ...
    def compose(self) -> ComposeResult:
        with Vertical(id="login"):
            ...
            yield Static(Text("Starting…", style=THEME.fg("muted")), id="login-status")
    ...
    def show_url(self, url: str, instructions: str) -> None:
        self._url = url
        self._set_status(instructions)
        self.query_one("#login-url", Static).update(Text(url, style=THEME.fg("accent")))

    def show_device_code(self, user_code: str, verification_uri: str) -> None:
        self._set_status(f"Open {verification_uri} and enter this code:")
        self.query_one("#login-url", Static).update(
            Text(user_code, style=THEME.fg("accent", bold=True))
        )
    ...
    def _set_status(self, message: str) -> None:
        self.query_one("#login-status", Static).update(Text(message, style=THEME.fg("muted")))
    ...
    def on_input_submitted(self, event: Input.Submitted) -> None:
        ...
        event.input.value = ""
        self._set_status("Exchanging the authorization code…")
