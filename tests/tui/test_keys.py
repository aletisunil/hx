...
from hx.keys import KEYMAP, action_name, display_key, load_keymap
...
def test_action_names_flatten_to_handler_names() -> None:
...
def test_an_action_carries_every_key_bound_to_it() -> None:
    """Both spellings have to reach the same action, or one of them is a key
    the user presses to no effect."""
    assert KEYMAP.keys_for("app.tools.expand") == ("ctrl+o", "ctrl+r")
...
def test_the_priority_flag_is_recorded_on_the_binding() -> None:
    """It marks the keys the session must check before the prompt sees them."""
    assert KEYMAP.bindings["app.selection.copy"].priority is True
    assert KEYMAP.bindings["app.clear"].priority is False
...
def test_the_prompt_spells_the_steer_key_the_way_help_does() -> None:
    ...
    no longer steered.
    """
    from hx.tui.views.prompt import placeholder_text as running_placeholder

    steer = KEYMAP.primary("tui.input.steer")

    assert f"{steer} steers" in running_placeholder(running=True)
    assert f"{steer} queues" in running_placeholder(running=True, enter_steers=True)
