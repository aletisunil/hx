...
from hx.tui.views.prompt import Candidate, Completion
...
def test_the_list_marks_the_selected_row(tmp_path: Path) -> None:
...
def test_nothing_is_drawn_when_there_is_nothing_to_show(tmp_path: Path) -> None:
    from hx.tui.views.prompt import Prompt
...
def test_descriptions_line_up_under_each_other(tmp_path: Path) -> None:
...
def test_path_completion_ignores_noise_directories(tmp_path: Path) -> None:
    from hx.tui.views.prompt import FileCompleter
...
def test_accepting_a_completion_keeps_the_rest_of_the_draft(tmp_path: Path) -> None:
    ...
    """
    from hx.tui.views.prompt import Prompt as PromptInput

    (tmp_path / "src").mkdir()
    prompt = PromptInput(tmp_path)
    prompt.text = "@sr and then fix the bug"
    prompt.buffer.cursor = 3
    ...
    assert prompt.buffer.cursor == 5, "the cursor lands after the completion, not at the end"
